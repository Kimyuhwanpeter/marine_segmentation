...
def cal_loss(model, images, labels, object_buf, bin):
    ...
    with tf.GradientTape() as tape: # background - 0; object - 1
        ...
        background_labels = tf.where(batch_labels == 0, 1, 0)

        m = max(object_buf[0], object_buf[1])
        if bin[0] != 0 and bin[1] != 0:
            background_loss = tf.keras.losses.BinaryCrossentropy(from_logits=True)(background_labels, b_logits)
            object_loss = structure_loss(labels, object_logits) + true_dice_loss(batch_labels, o_logits)

            total_loss = background_loss * object_buf[0] + object_loss * object_buf[1]

        if bin[0] != 0 and bin[1] == 0:
            background_loss = tf.keras.losses.BinaryCrossentropy(from_logits=True)(background_labels, b_logits) * object_buf[0]
